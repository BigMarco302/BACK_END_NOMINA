...
import com.seph_worker.worker.model.CoreSystem.ModulosDTO;
import com.seph_worker.worker.repository.Core.UserRoleModule.ModuleRepository;
import com.seph_worker.worker.repository.Core.UserRoleModule.RoleModuleRepository;
...
public class ModuleService {
    ...
    public WebServiceResponse addModule(ModulosDTO dto){
        try {
            CoreModules modulos = new CoreModules();
            ...
            if(dto.getParentId() == 0 ){
                CoreModules newModulos = moduleRepository.findById(modulos.getId()).orElse(null);
    ...
    public WebServiceResponse updateModule(ModulosDTO dto, Integer moduleId) {
        ...
        CoreModules modulos = moduleRepository.findById(moduleId).orElseThrow(() -> new ResourceNotFoundException("No se encontro el modulo"));
    ...
    public WebServiceResponse softdeleted(Integer moduleId){
        CoreModules module = moduleRepository.findById(moduleId).orElseThrow(()->new ResourceNotFoundException("No se encontrro el modulo"));
